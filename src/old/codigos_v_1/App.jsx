...
const BUILTIN = {
  "Vehículos": [
    {slug:"auto_medio",label:"Auto mediano",w:40,h:70},{slug:"auto_chico",label:"Auto chico",w:36,h:65},
    {slug:"car_mid",label:"Auto mediano 2",w:40,h:70},{slug:"car_compact",label:"Auto compacto",w:36,h:65},
    {slug:"car_veh2",label:"Auto top-view",w:40,h:70},{slug:"van",label:"Camioneta",w:50,h:70},
    {slug:"minivan",label:"Minivan",w:50,h:70},{slug:"pickup",label:"Pickup",w:44,h:70},
    {slug:"trailer",label:"Trailer",w:100,h:45},{slug:"truck_veh",label:"Camión",w:100,h:45},
    {slug:"camion_vuelco",label:"Camión vuelco",w:70,h:55},{slug:"grua",label:"Grúa",w:90,h:45},
    {slug:"moto_arriba",label:"Moto",w:30,h:60},{slug:"volcado",label:"Veh. volcado",w:70,h:50},
    {slug:"persona",label:"Persona",w:25,h:55},
  ],
  "Calles y vías": [
    {slug:"road_cross_2",label:"Cruce 2 carriles",w:120,h:120},{slug:"road_4lane_v",label:"4 carriles vert.",w:60,h:120},
    {slug:"road_t_top",label:"T-inter. arriba",w:120,h:90},{slug:"road_t_bot",label:"T-inter. abajo",w:120,h:90},
    {slug:"road_curve_2l",label:"Curva 2c izq.",w:100,h:100},{slug:"road_curve_2r",label:"Curva 2c der.",w:100,h:100},
    {slug:"road_curve_4r",label:"Curva 4c der.",w:100,h:100},{slug:"road_y",label:"Y-intersec.",w:100,h:100},
    {slug:"road_3junc",label:"Bifurc. 3",w:100,h:100},{slug:"road_junc_l",label:"Empalme izq.",w:100,h:100},
    {slug:"road_junc_r",label:"Empalme der.",w:100,h:100},{slug:"railroad_cross",label:"Cruce ferrov.",w:100,h:100},
  ],
  "Señales": [
    {slug:"sign_stop",label:"PARE",w:40,h:40},{slug:"sign_traffic_light",label:"Semáforo",w:30,h:55},
    {slug:"sign_all_way",label:"Todos pasan",w:40,h:40},{slug:"sign_wrong_way",label:"Contramano",w:40,h:40},
    {slug:"sign_left",label:"Doblar izq.",w:40,h:40},{slug:"sign_right",label:"Doblar der.",w:40,h:40},
    {slug:"sign_ped_ahead",label:"Peatones",w:40,h:40},{slug:"sign_school",label:"Escuela",w:40,h:40},
    {slug:"sign_curve",label:"Curva",w:40,h:40},{slug:"sign_two_way",label:"Doble mano",w:40,h:40},
  ],
  "Entorno": [
    {slug:"tree",label:"Árbol",w:40,h:50},{slug:"fir",label:"Pino",w:35,h:55},
    {slug:"house",label:"Casa",w:55,h:50},{slug:"gas_station",label:"Est. servicio",w:55,h:50},
...
function LibItem({label,src,onAdd}) {
  return(
    <div className="lib-item" onClick={onAdd}>
...
export default function App() {
  ...
  useEffect(() => {
    const h = e => {
      ...
      if ((e.key==="Delete"||e.key==="Backspace") && selId) delSel();
      if (e.key==="Escape") { setSelId(null); setPreview(null); }
    };
    window.addEventListener("keydown", h);
    return () => window.removeEventListener("keydown", h);
  }, [selId, hi, hist, undo, redo]);
  ...
  const addEl = item => {
    ...
    const e = { id:uid(), slug:item.slug, src:item.src, dataUrl:item.dataUrl,
      ...
      x: snap ? snapV(CW/2-w/2) : CW/2-w/2,
      y: snap ? snapV(CH/2-h/2) : CH/2-h/2,
      w, h, r:0 };
  ...
  const loadF=()=>{
    ...
    i.onchange=e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();
      r.onload=ev=>{try{const d=JSON.parse(ev.target.result);if(d.els){setEls(d.els);pushH(d.els);setSelId(null);}}catch{}};
              ...
              <div key={g} style={{marginBottom:3}}>
                ...
                {isOpen(g)&&(<div style={{paddingLeft:6,paddingBottom:4}}>
                  {items.map(it=><LibItem key={it.slug} label={it.label}
                    src={it.src||`/assets/lib/${it.slug}.png`} onAdd={()=>addEl(it)}/>)}
                </div>)}
              </div>
            ))}
            {dynLibs.map(lib=>(
              <div key={lib.name} style={{marginBottom:3}}>
                ...
                {isOpen(lib.name)&&(<div style={{paddingLeft:6,paddingBottom:4}}>
                  {lib.shapes.map((s,i)=><LibItem key={i} label={s.name} src={s.dataUrl}
                    onAdd={()=>addEl({label:s.name,dataUrl:s.dataUrl,w:60,h:60})}/>)}
                </div>)}
              </div>
            ))}
            {dynLibs.length===0&&(
